...
public class Main {
    ...
    public static stopWatchX timerText = new stopWatchX(Config.REFRESH_TEXT_RATE);
...
//		if (isImageDrawn)
        ...
        System.out.println(currentTextIndex);
        ctrl.drawString(100, 350,textMap.get("string"+currentTextIndex), c);
        if(timerText.isTimeUp()) {
                timerText.resetWatch();
                currentTextIndex++;
                if(currentTextIndex == textMap.size()) {
                    currentTextIndex = 0;
                }
        }
